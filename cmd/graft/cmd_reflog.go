...
func newReflogCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if headExact && ref != "" {
				return fmt.Errorf("--head cannot be combined with an explicit ref")
			}
			if headExact && strings.TrimSpace(entityFilter) != "" {
				return fmt.Errorf("--head cannot be combined with --entity")
			}

			// When --entity is provided, use entity-aware reader and filter.
			if strings.TrimSpace(entityFilter) != "" {
				return runReflogEntity(cmd, r, ref, entityFilter, limit, jsonOutput)
			}

			entries, err := readReflogEntries(r, ref, headExact, limit)
			if err != nil {
				return err
			}
			if jsonOutput {
				return writeJSON(cmd.OutOrStdout(), reflogEntriesToJSON(reflogDisplayRef(ref, headExact), entries, nil))
			}

			printReflogEntries(cmd.OutOrStdout(), entries)
			return nil
		},
	}
	...
	cmd.Flags().BoolVar(&headExact, "head", false, "show the worktree-local HEAD reflog instead of the current branch reflog")
	return cmd
}

func readReflogEntries(r *repo.Repo, ref string, headExact bool, limit int) ([]repo.ReflogEntry, error) {
	if headExact {
		return r.ReadHEADReflog(limit)
	}
	return r.ReadReflog(ref, limit)
}

func printReflogEntries(out io.Writer, entries []repo.ReflogEntry) {
	for _, e := range entries {
		entry := reflogEntryToJSON(e, nil)
		fmt.Fprintf(out, "%s %s %s %s\n", entry.ShortHash, entry.Date, e.Ref, e.Reason)
	}
}
...
func runReflogEntity(cmd *cobra.Command, r *repo.Repo, ref, entityFilter string, limit int, jsonOutput bool) error {
	...
	out := cmd.OutOrStdout()
	shown := 0
	var jsonEntries []JSONReflogEntry
	...
	for _, e := range allEntries {
		...
		if jsonOutput {
			jsonEntries = append(jsonEntries, reflogEntryToJSON(e.ReflogEntry, matched))
		} else {
			entry := reflogEntryToJSON(e.ReflogEntry, nil)
			fmt.Fprintf(out, "%s %s %s %s\n", entry.ShortHash, entry.Date, e.Ref, e.Reason)
	...
	if jsonOutput {
		return writeJSON(out, JSONReflogOutput{
			Ref:     reflogDisplayRef(ref, false),
			Entries: jsonEntries,
		})
	}
...
func reflogEntriesToJSON(ref string, entries []repo.ReflogEntry, entityByIndex map[int][]repo.ReflogEntityChange) JSONReflogOutput {
	out := JSONReflogOutput{
		Ref:     ref,
		Entries: make([]JSONReflogEntry, 0, len(entries)),
	}
	if out.Ref == "" && len(entries) > 0 {
		out.Ref = entries[0].Ref
	}
	for i, entry := range entries {
		out.Entries = append(out.Entries, reflogEntryToJSON(entry, entityByIndex[i]))
	}
	return out
}

func reflogEntryToJSON(entry repo.ReflogEntry, entities []repo.ReflogEntityChange) JSONReflogEntry {
	newHash := string(entry.NewHash)
	shortHash := newHash
	if len(shortHash) > 8 {
		shortHash = shortHash[:8]
	}
	jsonEntry := JSONReflogEntry{
		Ref:       entry.Ref,
		OldHash:   string(entry.OldHash),
		NewHash:   newHash,
		ShortHash: shortHash,
		Timestamp: entry.Timestamp,
		Date:      time.Unix(entry.Timestamp, 0).UTC().Format(time.RFC3339),
		Reason:    entry.Reason,
	}
	if len(entities) > 0 {
		jsonEntry.Entities = make([]JSONReflogEntityChange, 0, len(entities))
		for _, entity := range entities {
			jsonEntry.Entities = append(jsonEntry.Entities, JSONReflogEntityChange{
				Path:       entity.Path,
...
func reflogDisplayRef(ref string, headExact bool) string {
	if headExact {
		return "HEAD"
	}
